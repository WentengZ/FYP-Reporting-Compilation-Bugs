...
{
  return my_magic_123keyvalpairelement(list, node);
}

LinkNode
my_magic_123keyvalpairelement(LinkList list, LinkNode node)
{
    char *start, *end, *dat;
...
{
  return my_magic_123prefork(list, flags, ret_flags);
}

mod_export void
my_magic_123prefork(LinkList list, int flags, int *ret_flags)
{
	...
	if ((flags & (PREFORK_SINGLE|PREFORK_ASSIGN)) == PREFORK_ASSIGN &&
	    (insnode = my_magic_123keyvalpairelement(list, node))) {
	    node = insnode;
	    incnode(node);
	...
	if (isset(SHFILEEXPANSION)) {
	    ...
	    my_magic_123filesub(&cptr, flags & (PREFORK_TYPESET|PREFORK_ASSIGN));
	    /*
	...
	{
	    if (!(node = my_magic_123stringsubst(list, node,
    ...
    if (isset(SHFILEEXPANSION)) {
	/*
	 * my_magic_123stringsubst() may insert new nodes, so doesn't work
...
{
  return my_magic_123stringsubstquote(strstart, pstrdpos);
}

char *
my_magic_123stringsubstquote(char *strstart, char **pstrdpos)
{
    int len;
...
{
  return my_magic_123stringsubst(list, node, pf_flags, ret_flags, asssub);
}

LinkNode
my_magic_123stringsubst(LinkList list, LinkNode node, int pf_flags, int *ret_flags,
	    int asssub)
{
    int qt;
	...
	if ((qt = c == Qstring) || c == String) {
		...
		str2[-1] = *str = '\0';
		str = my_magic_123arithsubst(str + 2, &str3, str2);
		setdata(node, (void *) str3);
		continue;
	    } else if (c == Snull) {
		str3 = my_magic_123stringsubstquote(str3, &str);
		setdata(node, (void *) str3);
		continue;
	    } else {
		/*
		 ...
		 * recursive expansion via my_magic_123multsub()
		 ...
		 * previous two to signal my_magic_123paramsubst(), we'll do
	  ...
	  comsub: {
		...
		str[-1] = '\0';
		if (isset(EXECOPT))
		    str = my_magic_123arithsubst(str2 + 2, &str3, str+1);
		else
		    strncpy(str3, str2, 1);
		setdata(node, (void *) str3);
		continue;
...
{
  return my_magic_123quotesubst(str);
}

char *
my_magic_123quotesubst(char *str)
{
    char *s = str;
...
{
  return my_magic_123globlist(list, flags);
}

mod_export void
my_magic_123globlist(LinkList list, int flags)
{
    LinkNode node, next;
...
{
  return my_magic_123singsub(s);
}

mod_export void
my_magic_123singsub(char **s)
{
    local_list1(foo);

    init_list1(foo, *s);

    my_magic_123prefork(&foo, PREFORK_SINGLE, NULL);
    if (errflag)
	return;
    *s = (char *) ugetnode(&foo);
    DPUTS(nonempty(&foo), "BUG: my_magic_123singsub() produced more than one word!");
}
...
{
  return my_magic_123multsub(s, pf_flags, a, isarr, sep, ms_flags);
}

int
my_magic_123multsub(char **s, int pf_flags, char ***a, int *isarr, char *sep,
	int *ms_flags)
{
    int l;
    char **r, **p, *x = *s;
    ...
    my_magic_123prefork(&foo, pf_flags, ms_flags);
    if (errflag) {
	if (isarr)
	    *isarr = 0;
	return 0;
...
{
  return my_magic_123filesub(namptr, assign);
}

mod_export void
my_magic_123filesub(char **namptr, int assign)
{
    char *eql = NULL, *sub = NULL, *str, *ptr;
    int len;

    my_magic_123filesubstr(namptr, assign);
	...
	if ((*namptr)[1] && (eql = sub = strchr(*namptr + 1, Equals))) {
	    str = sub + 1;
	    if ((sub[1] == Tilde || sub[1] == Equals) && my_magic_123filesubstr(&str, assign)) {
		sub[1] = '\0';
	...
	if (sub > eql &&
	    ...
	    my_magic_123filesubstr(&str, assign)) {
	    sub[1] = '\0';
	    *namptr = dyncat(*namptr, str);
	}
	ptr = *namptr + len + 1;
...
{
  return my_magic_123equalsubstr(str, assign, nomatch);
}

char *
my_magic_123equalsubstr(char *str, int assign, int nomatch)
{
    char *pp, *cnam, *cmdstr;
...
{
  return my_magic_123filesubstr(namptr, assign);
}

mod_export int
my_magic_123filesubstr(char **namptr, int assign)
{
    char *str = *namptr;
	...
	if (isend(str[1])) {   /* ~ */
		...
		*namptr = dyncat(res, ptr2+1);
	    ...
	    if (val < 0)
		val = -val;
	    ds = my_magic_123dstackent(str[1], val);
	    if (!ds)
		return 0;
	...
	char *expn = my_magic_123equalsubstr(str+1, assign, isset(NOMATCH));
	if (expn) {
	    *namptr = expn;
	    return 1;
	}
...
{
  return my_magic_123strcatsub(d, pb, pe, src, l, s, glbsub, copied);
}

char *
my_magic_123strcatsub(char **d, char *pb, char *pe, char *src, int l, char *s, int glbsub,
	  int copied)
{
    char *dest;
    int pl = pe - pb;
...
char *
my_magic_123dopadding(char *str, int prenum, int postnum, char *preone, char *postone,
	  char *premul, char *postmul
#ifdef MULTIBYTE_SUPPORT
	  , int multi_width
#endif
...
{
  return my_magic_123get_strarg(s, lenp);
}

char *
my_magic_123get_strarg(char *s, int *lenp)
{
    convchar_t del;
    int len;
    char ctok = 0;
...
{
  return my_magic_123get_intarg(s, delmatchp);
}

int
my_magic_123get_intarg(char **s, int *delmatchp)
{
    int arglen;
    char *t = my_magic_123get_strarg(*s, &arglen);
    char *p, sav;
    zlong ret;
...
{
  return my_magic_123subst_parse_str(sp, single, err);
}

int
my_magic_123subst_parse_str(char **sp, int single, int err)
{
    char *s;
...
{
  return my_magic_123substevalchar(ptr);
}

char *
my_magic_123substevalchar(char *ptr)
{
...
{
  return my_magic_123untok_and_escape(s, escapes, tok_arg);
}

char *
my_magic_123untok_and_escape(char *s, int escapes, int tok_arg)
{
    int klen;
    char *dst = NULL;
...
{
  return my_magic_123check_colon_subscript(str, endp);
}

char *
my_magic_123check_colon_subscript(char *str, char **endp)
{
    int sav;
...
{
  return my_magic_123paramsubst(l, n, str, qt, pf_flags, ret_flags);
}

LinkNode
my_magic_123paramsubst(LinkList l, LinkNode n, char **str, int qt, int pf_flags,
	   int *ret_flags)
{
    char *aptr = *str, c, cc;
	...
	*str = s;
    ...
    DPUTS(c == '{', "BUG: inbrace == '{' in my_magic_123paramsubst()");
    /*
	...
	if ((c = *s) == '!' && s[1] != Outbrace && EMULATION(EMULATE_KSH)) {
		...
		case 'I':
		    s++;
		    flnum = my_magic_123get_intarg(&s, &dellen);
		    if (flnum < 0)
			goto flagerr;
		    s--;
		    break;
		...
		/* fall through */
		case 'j':
		    t = my_magic_123get_strarg(++s, &arglen);
		    if (*t) {
			sav = *t;
			*t = '\0';
			if (tt)
			    spsep = my_magic_123untok_and_escape(s + arglen,
						     escapes, tok_arg);
			else
			    sep = my_magic_123untok_and_escape(s + arglen,
						   escapes, tok_arg);
			*t = sav;
			s = t + arglen - 1;
		    } else
		...
		case 'r':
		    ...
		    /* delimiter position */
		    del0 = s;
		    num = my_magic_123get_intarg(&s, &dellen);
		    if (num < 0)
			goto flagerr;
		    if (tt)
			prenum = num;
		    ...
		    *t = '\0';
		    if (tt)
			premul = my_magic_123untok_and_escape(s + arglen, escapes,
						  tok_arg);
		    else
			postmul = my_magic_123untok_and_escape(s + arglen, escapes,
						   tok_arg);
		    *t = sav;
		    sav = *s;
		    s = t + arglen;
		    ...
		    *t = '\0';
		    if (tt)
			preone = my_magic_123untok_and_escape(s + arglen,
						  escapes, tok_arg);
		    else
			postone = my_magic_123untok_and_escape(s + arglen,
						   escapes, tok_arg);
		    *t = sav;
     ...
     * the current instance of my_magic_123paramsubst() doesn't fetch a value,
	...
	*s = 0;
	 ...
	 * my_magic_123multsub() (see below) but even so it would be nicer to pass down
	 ...
	 */
	if (my_magic_123multsub(&val, PREFORK_SUBEXP, (aspar ? NULL : &aval), &isarr, NULL,
		    &ms_flags) && quoted) {
	...
	*ret_flags |= MULTSUB_PARAM_NAME;
	...
	if ((isarr = v->isarr)) {
		 ...
		 * to avoid the my_magic_123multsub() horror.
		 */
     ...
     * is now in isarr.
     *
     * However, if we are replacing my_magic_123multsub() with something that
	...
	case Dash:
		...
		int split_flags;
		val = dupstring(s);
		/* If word-splitting is enabled, we ask my_magic_123multsub() to split
		...
		my_magic_123multsub(&val, split_flags, (aspar ? NULL : &aval),
			&isarr, NULL, &ms_flags);
		copied = 1;
		spbreak = 0;
	...
	case Equals:
		...
		*idend = '\0';
		...
		if (spsep || !arrasg) {
			...
			split_flags = PREFORK_NOSHWORDSPLIT;
		    my_magic_123multsub(&val, split_flags, &aval, &isarr, NULL,
			    &ms_flags);
		    spbreak = 0;
		}
		if (arrasg) {
...
colonsubscript:
	...
	if (colf && inbrace) {
	     ...
	     */
	    char *check_offset2;
	    char *check_offset = my_magic_123check_colon_subscript(s, &check_offset2);
	    if (check_offset) {
		...
		if (*check_offset2) {
		    char *nextp;
		    check_offset = my_magic_123check_colon_subscript(check_offset2 + 1,
							 &nextp);
		    if (check_offset) {
			check_offset2 = nextp;
			if (*check_offset2 && *check_offset2 != ':') {
	...
	if (colf) {
		...
		if (!isarr)
		    my_magic_123modify(&val, &s, inbrace);
		else {
		    char *ss;
		    char **ap = aval;
		    ...
		    while ((*pp = *ap++)) {
			ss = s;
			my_magic_123modify(pp++, &ss, inbrace);
		    }
		    if (pp == aval) {
			char *t = "";
			ss = s;
			my_magic_123modify(&t, &ss, inbrace);
		    }
		    s = ss;
		}
		copied = 1;
	...
	if (isarr) {
		...
		if (!(*av2ptr = my_magic_123substevalchar(*avptr))) {
		    haserr = 1;
		    break;
		}
	    }
	    *av2ptr = NULL;
	    ...
	    /* When noerrs = 1, the only error is out-of-memory */
	    if (!(val = my_magic_123substevalchar(val)))
		haserr = 1;
	}
	noerrs = one;
	if (!quoteerr) {
    ...
    if (isarr && ssub) {
	/* my_magic_123prefork() wants a scalar, so join no matter what else */
	...
	*fstr = '\0';
	...
	if (plan9) {
	    ...
	    *--fstr = Marker;
	    init_list1(tl, fstr);
	    if (!eval && !my_magic_123stringsubst(&tl, firstnode(&tl), ssub, ret_flags, 0))
		return NULL;
	    *str = aptr;
	    tn = firstnode(&tl);
	    while ((x = *aval++)) {
		if (prenum || postnum)
		    x = my_magic_123dopadding(x, prenum, postnum, preone, postone,
				  premul, postmul
#ifdef MULTIBYTE_SUPPORT
				  , multi_width
#endif
			);
		if (eval && my_magic_123subst_parse_str(&x, (qt && !nojoin), quoteerr))
		    return NULL;
		xlen = strlen(x);
		for (tn = firstnode(&tl);
		     tn && *(y = (char *) getdata(tn)) == Marker;
		     incnode(tn)) {
		    my_magic_123strcatsub(&y, ostr, aptr, x, xlen, y + 1, globsubst,
			      copied);
		    if (qt && !*y && isarr != 2)
		...
		return NULL;
	    xlen = strlen(x);
	    my_magic_123strcatsub(&y, ostr, aptr, x, xlen, NULL, globsubst, copied);
	    if (qt && !*y && isarr != 2)
		y = dupstring(nulstring);
	    setdata(n, (void *) y);
		...
		return NULL;
	    xlen = strlen(x);
	    *str = my_magic_123strcatsub(&y, aptr, aptr, x, xlen, fstr, globsubst, copied);
	    if (qt && !*y && isarr != 2)
		y = dupstring(nulstring);
	    insertlinknode(l, n, (void *) y), incnode(n);
	}
	...
	*str = getdata(n = on);
	...
	if (eval && my_magic_123subst_parse_str(&x, (qt && !nojoin), quoteerr))
	    return NULL;
	xlen = strlen(x);
	*str = my_magic_123strcatsub(&y, ostr, aptr, x, xlen, fstr, globsubst, copied);
	if (qt && !*y)
	    y = dupstring(nulstring);
	setdata(n, (void *) y);
    }
...
{
  return my_magic_123arithsubst(a, bptr, rest);
}

char *
my_magic_123arithsubst(char *a, char **bptr, char *rest)
{
    char *s = *bptr, *t;
    char buf[BDIGBUFSIZE], *b;
    mnumber v;

    my_magic_123singsub(&a);
    v = matheval(a);
...
{
  my_magic_123modify(str, ptr, inbrace);
}

void
my_magic_123modify(char **str, char **ptr, int inbrace)
{
	...
	*str = dupstring(*str);
	...
	for (; !c && **ptr;) {
		...
		*ptr = ptr3 - 1;
		...
		wall = 1;
		(*ptr)++;
		ptr1 = my_magic_123get_strarg(ptr2 = *ptr, &charlen);
		if ((sav = *ptr1))
		    *ptr1 = '\0';
		...
		*ptr = ptr1 + charlen;
	    ...
	    case 'F':
		(*ptr)++;
		rec = my_magic_123get_intarg(ptr, &dellen);
		break;
	    default:
		*ptr = lptr;
		return;
	...
	while (rec--) {
		...
		for (t = e = *str; (tt = findword(&e, sep));) {
		    ...
		    *e = tc;
		    ...
		    case 'c':
		    {
			char *copy2 = my_magic_123equalsubstr(copy, 0, 0);
			if (copy2)
			    copy = copy2;
			break;
		    }
		...
		case 'c':
		{
		    char *copy2 = my_magic_123equalsubstr(*str, 0, 0);
		    if (copy2)
			*str = copy2;
		    break;
		}
...
{
  return my_magic_123dstackent(ch, val);
}
