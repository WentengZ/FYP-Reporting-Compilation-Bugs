...
void extractEnums(const std::string &filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (!copying && trimmedLine.find("enum") == 0) {
            copying = true;
        }
        ...
        if (copying && line.find("};") != std::string::npos) {
            copying = false;
        }
    }
    OutputFile.close();
...
void addExternIfNeeded(std::string& str) {
    ...
    if (nonSpacePos != std::string::npos && nonSpacePos < str.length()) {
        ...
        if (firstWord == "static") {
          str = str.substr(nonSpacePos + 6, str.length() - nonSpacePos - 6);
        }
        if (firstWord != "extern") {
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    if (const VarDecl *VD = Result.Nodes.getNodeAs<VarDecl>("varDecl")) {
      if (VD->isFileVarDecl()) {
        ...
        size_t nonSpacePos = VarDelcCode.find_first_not_of(' ');
        if (nonSpacePos != std::string::npos && nonSpacePos < VarDelcCode.length()) {
        std::string firstWord = VarDelcCode.substr(nonSpacePos, 6); // Check the first 6 characters
        if (firstWord == "static") {
          VarDelcCode = VarDelcCode.substr(nonSpacePos + 6, VarDelcCode.length() - nonSpacePos - 6);
        }
        ...
        for (int func_i = 0; func_i < all_function_names.size(); func_i++) {
          if (all_function_names[func_i].compare(FD->getNameAsString()) == 0) {
            ...
            if (FunctionCode.find("#ifdef") == std::string::npos) {
              ...
              FunctionCode += (std::string)"my_magic_123" + all_function_names[func_i] +"(";
    ...
    if (const EnumDecl *ED = Result.Nodes.getNodeAs<EnumDecl>("enumDecl")) {
      ...
      std::string enumCode = getSourceCode(SM, SR);
      enumCode += "\n\n";
...
int main(int argc, const char *argv[]) {
  ...
  extractEnums(source_file_directory);
  firstRun = false;
  ...
  for (int i = 0; i < all_function_names.size(); i++) {
    ...
    //--Finish Spliting into three files, start running and testing using mixed-compiling---
